...
void chilgenb_slice_plots() {
  ...
  auto* syst_ptr = new MCC9SystematicsCalculator(
    ...
    "/uboone/data/users/chilgenb/post_process_jun2022/univmake_kinreco_1p.root",
    "systcalc.conf" );
  auto& syst = *syst_ptr;
  ...
  for ( size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx ) {
    ...
    std::string out1png = outname_base+"1.png";
    std::string out2png = outname_base+"2.png";
    c1->SaveAs(out1png.c_str());
    c2->SaveAs(out2png.c_str());

    std::string out1root = outname_base+"1.root";
    std::string out2root = outname_base+"2.root";
    c1->SaveAs(out1root.c_str());
    c2->SaveAs(out2root.c_str());
