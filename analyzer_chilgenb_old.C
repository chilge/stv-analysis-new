...
int multiplicity ( const int& pdg, const std::vector<int>& pdglist) {
  int n=0;
  for(const int& thispdg : pdglist)
    if(pdg==thispdg) n++;
...
constexpr float LEAD_P_MAX_MOM_CUT = 1.000; // GeV/c
...
//constexpr double NEUTRON_MASS = 0.93956541; // GeV these three are now defined in KinematicReco.hh
...
class AnalysisEvent {
  ...
  public:
    ...
    MyPointer< TLorentzVector > mc_p4_had_;
    MyPointer< TLorentzVector > p4_had_;
    ...
    // predicted muon kinematics using kinematic reconstruction method
    // see Phys. Rev. D 103, 112011 (2021)
    float kin_reco_enu_ = BOGUS;                // neutrino energy [GeV] using reconstructed inputs
    float kin_reco_enu_diff_ = BOGUS;           // reco neutrino [GeV] difference between prediction and reconstructed value (reco-true)
    float kin_reco_enu_frac_ = BOGUS;           // reco neutrino [GeV] fractional difference between prediction and reconstructed value (reco-true)/true
    float kin_reco_emu_ = BOGUS;                // muon energy [GeV] using reco inputs
    float kin_reco_pmu_ = BOGUS;                // muon momentum [GeV/c] using reconstructed inputs
    float kin_reco_costhetamu_ = BOGUS;         // cosine of muon polar angle using reconstructed inputs
    float kin_reco_thetamu_ = BOGUS;            // muon polar angle [rad] using reconstructed inputs
    float kin_reco_pmu_diff_ = BOGUS;           // reco muon momentum [GeV/c] difference between prediction and reconstructed value (reco-true)
    float kin_reco_pmu_frac_ = BOGUS;           // reco muon momentum [GeV/c] fractional difference between prediction and reconstructed value (reco-true)/true
    float kin_reco_costhetamu_diff_ = BOGUS;    // cosine of muon polar angle [rad;] difference between prediction and reconstructed value (reco-true)
    float kin_reco_thetamu_diff_ = BOGUS;       // muon polar angle [rad]; difference between prediction and reconstructed value (reco-true)
    float mc_kin_reco_enu_ = BOGUS;             // neutrino energy [GeV] using truth inputs
    float mc_kin_reco_enu_diff_ = BOGUS;        // true neutrino [GeV] difference between prediction and reconstructed value (reco-true)
    float mc_kin_reco_enu_frac_ = BOGUS;        // true neutrino [GeV] fractional difference between prediction and reconstructed value (reco-true)/true
    float mc_kin_reco_emu_ = BOGUS;             // muon energy [GeV] using truth inputs
    float mc_kin_reco_pmu_ = BOGUS;             // muon momentum [GeV/c] using truth inputs
    float mc_kin_reco_costhetamu_ = BOGUS;      // cosine of muon polar angle using truth inputs
    float mc_kin_reco_thetamu_ = BOGUS;         // muon polar angle [rad] using truth inputs
    float mc_kin_reco_pmu_diff_ = BOGUS;        // true muon momentum [GeV/c]; difference between prediction and reconstructed value (reco-true)
float mc_kin_reco_pmu_frac_ = BOGUS;        // true muon momentum [GeV/c]; fractional difference between prediction and reconstructed value (reco-true)/true
    float mc_kin_reco_costhetamu_diff_ = BOGUS; // true cosine pf muon polar angle [rad]; difference between prediction and reconstructed value (reco-true)
    float mc_kin_reco_thetamu_diff_ = BOGUS;    // true muon polar angle [rad]; difference between prediction and reconstructed value (reco-true)
    int   np_ = BOGUS;
    int   nn_ = BOGUS;
    int   mc_np_ = BOGUS;
    int   mc_nn_ = BOGUS;
...
void set_event_branch_addresses(TTree& etree, AnalysisEvent& ev)
{
  ...
  if ( has_shower_branches ) {
    ...
    // When the shower information is not available, delete the ownedstd::vectors
  ...
  bool create = false)
{
  ...
  if ( ev.mc_weights_map_ ) {
    ...
    for ( auto& pair : *ev.mc_weights_map_ ) {
      ...
      // Prepend "weight_" to the name of thestd::vector of weights in the map
      std::string weight_branch_name = "weight_" + pair.first;

      // Store a pointer to thestd::vector of weights (needed to set the branch
      // address properly) in the temporary map of pointers
      ev.mc_weights_ptr_map_[ weight_branch_name ] = &pair.second;

      // Set the branch address for thisstd::vector of weights
  ...
  // Index for the muon candidate in thestd::vectors of PFParticles
  set_output_branch_address( out_tree, "muon_candidate_idx",
    &ev.muon_candidate_idx_, create, "muon_candidate_idx/I" );

  // Index for the leading proton candidate in thestd::vectors of PFParticles
  ...
  // Reco 4-momentum of final state hadronic system
  set_object_output_branch_address< TLorentzVector >( out_tree,
    "p4_had", ev.p4_had_, create );
  ...
  // True 4-momentum of final state hadronic system
     set_object_output_branch_address< TLorentzVector >( out_tree,
         "mc_p4_had", ev.mc_p4_had_, create );
  ...
  set_output_branch_address( out_tree, "kin_reco_enu",
    &ev.kin_reco_enu_, create, "kin_reco_enu/F" );

  set_output_branch_address( out_tree, "kin_reco_enu_diff",
    &ev.kin_reco_enu_diff_, create, "kin_reco_enu_diff/F" );

  set_output_branch_address( out_tree, "kin_reco_enu_frac",
    &ev.kin_reco_enu_frac_, create, "kin_reco_enu_frac/F" );

  set_output_branch_address( out_tree, "kin_reco_pmu",
    &ev.kin_reco_pmu_, create, "kin_reco_pmu/F" );

  set_output_branch_address( out_tree, "kin_reco_emu",
    &ev.kin_reco_emu_, create, "kin_reco_emu/F" );

  set_output_branch_address( out_tree, "kin_reco_costhetamu",
    &ev.kin_reco_costhetamu_, create, "kin_reco_costhetamu/F" );

  set_output_branch_address( out_tree, "kin_reco_thetamu",
    &ev.kin_reco_thetamu_, create, "kin_reco_thetamu/F" );

  set_output_branch_address( out_tree, "kin_reco_pmu_diff",
    &ev.kin_reco_pmu_diff_, create, "kin_reco_pmu_diff/F" );

  set_output_branch_address( out_tree, "kin_reco_pmu_frac",
    &ev.kin_reco_pmu_frac_, create, "kin_reco_pmu_frac/F" );

  set_output_branch_address( out_tree, "kin_reco_costhetamu_diff",
    &ev.kin_reco_costhetamu_diff_, create, "kin_reco_costhetamu_diff/F" );

  set_output_branch_address( out_tree, "kin_reco_thetamu_diff",
    &ev.kin_reco_thetamu_diff_, create, "kin_reco_thetamu_diff/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_enu",
    &ev.mc_kin_reco_enu_, create, "mc_kin_reco_enu/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_enu_diff",
    &ev.mc_kin_reco_enu_diff_, create, "mc_kin_reco_enu_diff/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_enu_frac",
    &ev.mc_kin_reco_enu_frac_, create, "mc_kin_reco_enu_frac/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_pmu",
    &ev.mc_kin_reco_pmu_, create, "mc_kin_reco_pmu/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_emu",
    &ev.mc_kin_reco_emu_, create, "mc_kin_reco_emu/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_costhetamu",
    &ev.mc_kin_reco_costhetamu_, create, "mc_kin_reco_costhetamu/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_thetamu",
    &ev.mc_kin_reco_thetamu_, create, "mc_kin_reco_thetamu/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_pmu_diff",
    &ev.mc_kin_reco_pmu_diff_, create, "mc_kin_reco_pmu_diff/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_pmu_frac",
    &ev.mc_kin_reco_pmu_frac_, create, "mc_kin_reco_pmu_frac/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_costhetamu_diff",
    &ev.mc_kin_reco_costhetamu_diff_, create, "mc_kin_reco_costhetamu_diff/F" );

  set_output_branch_address( out_tree, "mc_kin_reco_thetamu_diff",
    &ev.mc_kin_reco_thetamu_diff_, create, "mc_kin_reco_thetamu_diff/F" );

  set_output_branch_address( out_tree, "np",
    &ev.np_, create, "np/I" );
  set_output_branch_address( out_tree, "nn",
    &ev.np_, create, "np/I" );
  set_output_branch_address( out_tree, "mc_np",
    &ev.mc_np_, create, "mc_np/I" );
  set_output_branch_address( out_tree, "mc_nn",
    &ev.mc_nn_, create, "mc_nn/I" );
  ...
  const std::string& output_filename)
{
  ...
  while ( true ) {
    ...
    out_tree->Fill();
    ++events_entry;
    //if(events_entry>1000) break; //useful for development
  }
...
// Sets the index of the muon candidate in the trackstd::vectors, or BOGUS_INDEX if
...
void AnalysisEvent::compute_observables() {
  ...
  //re-initialize kin reco vars
  kin_reco_enu_             = BOGUS;
  kin_reco_enu_diff_        = BOGUS;
  kin_reco_enu_frac_        = BOGUS;
  kin_reco_pmu_             = BOGUS;
  kin_reco_emu_             = BOGUS;
  kin_reco_costhetamu_      = BOGUS;
  kin_reco_thetamu_         = BOGUS;
  kin_reco_pmu_diff_        = BOGUS;
  kin_reco_pmu_frac_        = BOGUS;
  kin_reco_costhetamu_diff_ = BOGUS;
  kin_reco_thetamu_diff_    = BOGUS;
  p4_had_->SetXYZT(BOGUS,BOGUS,BOGUS,BOGUS);
  ...
  // Reset thestd::vector of reconstructed proton candidate 3-momenta
  p3_p_vec_->clear();
  ...
  if ( muon && lead_p ) {
    //if(sel_CCNp0pi_)std::cout << "found a CCNp0pi candidate!" <<std::endl;
    for ( int p = 0; p < num_pf_particles_; ++p ) {
      ...
      if(KEp<0) {
         if(sel_CCNp0pi_)
          std::cout << "negative proton KE! " << KEp
              << "\n\tdir_x: " << p_dirx
              << "\n\tdir_y: " << p_diry
              << "\n\tdir_z: " << p_dirz <<std::endl;
         continue;
      }
      float p_mom = real_sqrt( KEp*KEp + 2.*PROTON_MASS*KEp );
    ...
    np_ = p3_p_vec_->size();
    nn_ = multiplicity(2112,*pfp_reco_pdg_);

    // good place to calculate the kinematic reco variables
    TLorentzVector hadvec     = GetHadVecW(*p3_p_vec_,false); //pass true to fix W=Mp
    p4_had_->SetXYZT(hadvec.X(),hadvec.Y(),hadvec.Z(),hadvec.T());
    if(hadvec.M()==0)std::cout << "warning: reco inputs hadvec is empty!" <<std::endl;
    kin_reco_enu_             = KinRecoEnu(hadvec);
    kin_reco_enu_diff_        = kin_reco_enu_ - mc_nu_energy_; // note neutrino energy from pndr N/A
    kin_reco_enu_frac_        = kin_reco_enu_diff_/mc_nu_energy_;
    kin_reco_pmu_             = KinRecoMom(hadvec);
    if(kin_reco_pmu_==BOGUS) {
     std::cout << "kin_reco_pmu set but still has BOGUS value" <<std::endl;
     std::cout << "\thadvec: " << "|(" << hadvec.X()
            << ", " << hadvec.Y() << ", "
            << hadvec.Z() << ", " << hadvec.T()
            << ")|^2 = " << hadvec.M() <<std::endl;
    }
    kin_reco_emu_             = KinRecoE(hadvec);
    kin_reco_costhetamu_      = KinRecoCosTheta(hadvec);
    kin_reco_thetamu_         = KinRecoTheta(hadvec);
    kin_reco_pmu_diff_        = kin_reco_pmu_ - p3_mu_->Mag();
    kin_reco_pmu_frac_        = kin_reco_pmu_diff_/p3_mu_->Mag();
    kin_reco_costhetamu_diff_ = kin_reco_costhetamu_ - p3_mu_->CosTheta();
    kin_reco_thetamu_diff_    = kin_reco_thetamu_ - p3_mu_->Theta();


    // TODO: reduce code duplication by just getting the leading proton
    // 3-momentum from this sortedstd::vector
...
void AnalysisEvent::compute_mc_truth_observables() {
  ...
  // Reset thestd::vector of true MC proton 3-momenta
  mc_p3_p_vec_->clear();
  std::vector<TVector3> mc_p3_p_vec_thresh;
  mc_p4_had_->SetXYZT(BOGUS,BOGUS,BOGUS,BOGUS);
  ...
  for ( int p = 0; p < num_mc_daughters; ++p ) {
    ...
    if ( pdg == PROTON )
    {
      ...
      if(temp_p3.Mag()>0.25)//only use "reconstructable in principle" protons for kin reco [GeV/c] (same as base selection for leading proton)
      //if(temp_p3.Mag()>0.35)//only use "reconstructable in principle" protons (harsher than base selection)
       mc_p3_p_vec_thresh.push_back(temp_p3);
  ...
  mc_np_ = mc_p3_p_vec_->size();
  mc_nn_ = multiplicity(2112, *mc_nu_daughter_pdg_); 

  // good place to calculate the kinematic reco vars
  mc_kin_reco_enu_             = BOGUS;
  mc_kin_reco_enu_diff_        = BOGUS;
  mc_kin_reco_enu_frac_        = BOGUS;
  mc_kin_reco_pmu_             = BOGUS;
  mc_kin_reco_emu_             = BOGUS;
  mc_kin_reco_costhetamu_      = BOGUS;
  mc_kin_reco_thetamu_         = BOGUS;
  mc_kin_reco_pmu_diff_        = BOGUS;
  mc_kin_reco_pmu_frac_        = BOGUS;
  mc_kin_reco_costhetamu_diff_ = BOGUS;
  mc_kin_reco_thetamu_diff_    = BOGUS;

  //TLorentzVector hadvec        = GetHadVec(*mc_p3_p_vec_);
  //TLorentzVector hadvec        = GetHadVec(mc_p3_p_vec_thresh);
  TLorentzVector hadvec = GetHadVecW(*mc_p3_p_vec_,false); // pass true to use fixed W=mp
  mc_p4_had_->SetXYZT(hadvec.X(),hadvec.Y(),hadvec.Z(),hadvec.T());
  /*if(hadvec.M()<0){
   std::cout << "negative W with " << num_mc_daughters << " FS particles:" <<std::endl;
    for ( size_t d = 0u; d < num_mc_daughters; ++d ) {
     std::cout << "\t" << mc_nu_daughter_pdg_->at( d ) <<std::endl;
    }
  }*/

  if(hadvec.M() != 0) {
    mc_kin_reco_enu_             = KinRecoEnu(hadvec);
    mc_kin_reco_enu_diff_        = mc_kin_reco_enu_ - mc_nu_energy_;
    mc_kin_reco_enu_frac_        = mc_kin_reco_enu_diff_/mc_nu_energy_;
    mc_kin_reco_pmu_             = KinRecoMom(hadvec);
    mc_kin_reco_emu_             = KinRecoE(hadvec);
    mc_kin_reco_costhetamu_      = KinRecoCosTheta(hadvec);
    mc_kin_reco_thetamu_         = KinRecoTheta(hadvec);
    mc_kin_reco_pmu_diff_        = mc_kin_reco_pmu_ - mc_p3_mu_->Mag();
    mc_kin_reco_pmu_frac_        = mc_kin_reco_pmu_diff_/mc_p3_mu_->Mag();
    mc_kin_reco_costhetamu_diff_ = mc_kin_reco_costhetamu_ - mc_p3_mu_->CosTheta();
    mc_kin_reco_thetamu_diff_    = mc_kin_reco_thetamu_ - mc_p3_mu_->Theta();
  }
  else if(mc_is_signal_) {
   std::cout << "WARNING MC signal event with W=0!" <<std::endl;
  }
  else if(mc_p3_p_vec_->size()>0) {
  //else if(mc_p3_p_vec_thresh.size()>0){
   std::cout << "WARNING: we have " << mc_p3_p_vec_->size() << " protons but W=0!" <<std::endl;
   std::cout << "\thadvec: " << "|(" << hadvec.X()
                << ", " << hadvec.Y() << ", "
                << hadvec.Z() << ", " << hadvec.T()
                << ")|^2 = " << hadvec.M() <<std::endl;
    TVector3 tmpvec;
    for(TVector3 const& vec : *mc_p3_p_vec_) {
     std::cout << "\tmc proton momentum " << vec.Mag() << " (" << vec.X()
                << ", " << vec.Y() << ", "
                << vec.Z() << ")" << std::endl;
      tmpvec+=vec;
    }
   std::cout << "locally calculated P_h 3-vector " << tmpvec.Mag() << " (" << tmpvec.X()
                << ", " << tmpvec.Y() << ", "
                << tmpvec.Z() << ")" << std::endl;
  }
