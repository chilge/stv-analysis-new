...
//
// Additions for Kin Reco analysis 23 September 2022
// Chris Hilgenberg <chilgenb@umn.edu>
...
// helper function for calculating multiplicity given a PDG code and
// a list of pdg codes
...
class AnalysisEvent {
  ...
  public:
    ...
    bool mc_sublead_p_in_mom_range_ = false;
    ...
    // Whether the sub-leading proton candidate(s) has(have) a range-based reco momentum
    // above LEAD_P_MIN_MOM_CUT and below LEAD_P_MAX_MOM_CUT
    bool sel_sublead_p_passed_mom_cuts_ = false;
...
float mc_kin_reco_pmu_frac_ = BOGUS;        // true muon momentum [GeV/c]; fractional difference between prediction and reconstructed value (reco-true)/true
    ...
    int   np_ = BOGUS; // number of protons in with reconstructed momenta in lead proton range
    int   nn_ = BOGUS; // number of neutrons (no threshold) / not sure if diff from mc_nn
    int   mc_np_ = BOGUS; // number of protons in with true momenta in lead proton range
    int   mc_nn_ = BOGUS; // number of neutrons (no threshold) 
  ...
  bool create = false)
{
  ...
  set_output_branch_address( out_tree, "mc_sublead_p_in_mom_range",
    &ev.mc_sublead_p_in_mom_range_, create, "mc_sublead_p_in_mom_range/O" );
  ...
  set_output_branch_address( out_tree, "sel_sublead_p_passed_mom_cuts",
    &ev.sel_sublead_p_passed_mom_cuts_, create, "sel_sublead_p_passed_mom_cuts/O" );
  ...
  // kinematic reconstruction variables- reco input
  ...
  // kinematic reconstruction variables- truth input
...
void AnalysisEvent::compute_observables() {
  ...
  sel_sublead_p_passed_mom_cuts_ = false;
  kin_reco_enu_             = BOGUS;
  ...
  if ( lead_p ) {
    sel_sublead_p_passed_mom_cuts_= true;
  ...
  if ( muon && lead_p ) {
    ...
    for ( int p = 0; p < num_pf_particles_; ++p ) {
      ...
      float p_mom = real_sqrt( KEp*KEp + 2.*PROTON_MASS*KEp );
      if(p_mom < LEAD_P_MIN_MOM_CUT || p_mom > LEAD_P_MAX_MOM_CUT)
        sel_sublead_p_passed_mom_cuts_ = false;
  ...
  if ( muon && lead_p ) {
   ...
   // good place to calculate the kinematic reco variables
    TLorentzVector hadvec     = GetHadVecW(*p3_p_vec_,true); //pass true to fix W=Mp
...
void AnalysisEvent::compute_mc_truth_observables() {
  ...
  //re-initialize kin reco varsi
  mc_sublead_p_in_mom_range_ = true;
  ...
  for ( int p = 0; p < num_mc_daughters; ++p ) {
    ...
    if ( pdg == PROTON )
    {
      ...
      if(std::isinf(mom)) std::cout << "inifinite mc momentum!" << std::endl;
      if(mom < LEAD_P_MIN_MOM_CUT || mom > LEAD_P_MAX_MOM_CUT)
        mc_sublead_p_in_mom_range_ = false;
      if ( mom > max_mom ) {
        max_mom = mom;
  ...
  TLorentzVector hadvec = GetHadVecW(*mc_p3_p_vec_,true);
