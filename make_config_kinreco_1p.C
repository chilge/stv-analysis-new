...
void make_config_kinreco_1p() {
  ...
  std::map< std::string, std::vector<double> > kinreco_bin_edge_map = {
    { "kin_reco_pmu_diff", {-2,-0.8,-0.5,-0.3,-0.15,-0.05,0.02,0.12,0.63,3} },
    { "kin_reco_costhetamu_diff", {-2,-1,-0.63,-0.38,-0.179,-0.033,0.076,0.195,0.36,0.58,0.95,2} },
    { "kin_reco_enu", {-4,-1.8,0,0.4,0.55,0.75,0.95,1.3,2,4}}
  };
  ...
  for ( const auto& pair : kinreco_bin_edge_map ) {
    ...
    if(pair.first != "kin_reco_enu"){
      selection_tmp += " && kin_reco_enu > 0";
      signal_tmp += " && mc_kin_reco_enu > 0";
    }
    ...
    cout << "act_var_idx = " << act_var_idx << endl;
  ...
  std::ofstream out_file( "myconfig_kinreco_1p.txt" );
  ...
  std::ofstream sb_file( "mybins_kinreco_1p.txt" );
  sb_file << sb;
}
