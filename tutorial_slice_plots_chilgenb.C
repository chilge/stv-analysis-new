...
void tutorial_slice_plots() {
    ...
    fpm.load_file_properties( "nuwro_file_properties_chilgenb.txt" );
  #endif

  auto* syst_ptr = new MCC9SystematicsCalculator(
    //"/uboone/data/users/chilgenb/post_process_jun2022/univmake_kinreco_np.root",
    "/uboone/data/users/chilgenb/univmake_kinreco_1p.root",
    "systcalc.conf" );
  auto& syst = *syst_ptr;
  ...
  auto* sb_ptr = new SliceBinning( "mybins_kinreco_1p.txt"); //tutorial_slice_config.txt" );
  auto& sb = *sb_ptr;

  for ( size_t sl_idx = 0u; sl_idx < sb.slices_.size(); ++sl_idx ) {

    const auto& slice = sb.slices_.at( sl_idx );
    std::string outname_base = "kinreco_1p_slice"+std::to_string(sl_idx)+"_c";
    ...
    BinWidthNormalize(slice_bnb->hist_.get());
    BinWidthNormalize(slice_ext->hist_.get());
    BinWidthNormalize(slice_mc_plus_ext->hist_.get());
    ...
    for ( auto iter = cat_map.crbegin(); iter != cat_map.crend(); ++iter )
    {
      ...
      BinWidthNormalize(temp_slice_mc->hist_.get());
    ...
    std::string out1 = outname_base+"1.png";
    std::string out2 = outname_base+"2.png";
