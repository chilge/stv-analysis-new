...
//
// Modified for used with 3D STV variables by
// Chris Hilgenberg and Andy Furmanski
  ...
  bool create = false)
{
  ...
  set_output_branch_address( out_tree, "delta_phi3d",
    &ev.delta_phi3d_, create, "delta_phi3d/F" );
  ...
  set_output_branch_address( out_tree, "mc_delta_phi3d",
    &ev.mc_delta_phi3d_, create, "mc_delta_phi3d/F" );
...
void AnalysisEvent::compute_observables() {
  ...
  if ( muon && lead_p ) {
    ...
    delta_alpha3d = CalcAlpha3D(p3mu[0],p3mu[1],p3mu[2],p3p[0],p3p[1],p3p[2]); 
    delta_phi3d = CalcPhi3D(p3mu[0],p3mu[1],p3mu[2],p3p[0],p3p[1],p3p[2]);
  }
}
...
void AnalysisEvent::compute_mc_truth_observables() {
  ...
  if ( true_muon && true_lead_p ) {
    ...
    mc_delta_alpha3d = CalcAlpha3D(mc_p3mu[0],mc_p3mu[1],mc_p3mu[2],mc_p3p[0],mc_p3p[1],mc_p3p[2]);
    mc_delta_phi3d = CalcPhi3D(mc_p3mu[0],mc_p3mu[1],mc_p3mu[2],mc_p3p[0],mc_p3p[1],mc_p3p[2]);
